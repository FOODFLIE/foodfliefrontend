...
export const generateLocationSEO = (
  ...
  const keywords = [
    ...
    `15 Minute delivery ${location}`,
    ...cuisines.map((cuisine) => `${cuisine} delivery ${location}`),
    ...restaurants
      .slice(0, 5)
      .map((restaurant) => `${restaurant.name} ${location}`),
  ].join(", ");

  return { title, description, keywords };
};

export const generateDishSEO = (
  dishName,
  location = "Hyderabad",
  restaurants = [],
) => {
  const title = `${dishName} Delivery in ${location} | Order Online | FoodFlie`;
  const description = `Craving ${dishName}? Order from ${
    restaurants.length > 0
      ? restaurants
          ...
          .join(", ")
      : "top restaurants"
  } in ${location}. Get ${dishName} delivered in 15 minutes at menu prices.`;
  const keywords = [
    `${dishName} delivery`,
...
export const generateCuisineSEO = (
  cuisine,
  location = "Hyderabad",
  restaurants = [],
) => {
...
export const generateCategorySEO = (category, location = "Hyderabad") => {
  ...
  const description = `Order ${categoryName.toLowerCase()} online in ${location} and get delivery in 15 minutes. Fresh ${categoryName.toLowerCase()} from top restaurants at menu prices.`;
  const keywords = [
...
export const generateRestaurantKeywords = (
  restaurantName,
  location = "Hyderabad",
  cuisines = [],
) => [
...
export const generateLocalMetaDescription = (
  type,
  name,
  location = "Hyderabad",
) => {
  const templates = {
    restaurant: `Order from ${name} in ${location} and get delivery in 15 minutes. Menu prices guaranteed, no hidden charges. Cash on delivery available.`,
    dish: `Craving ${name}? Get it delivered in ${location} in just 15 minutes. Order from top restaurants at menu prices.`,
    cuisine: `Order ${name} food in ${location} with 15-minute delivery. Authentic flavors from top restaurants at menu prices.`,
    location: `Food delivery in ${name} in just 15 minutes. Order from 100+ restaurants. Menu prices guaranteed, no hidden charges.`,
  };
...
export const generateLocalPageSchema = (location, restaurants = []) => ({
  ...
  description: `Food delivery restaurants in ${location} with 15-minute delivery`,
