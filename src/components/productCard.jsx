...
const ProductCard = ({
  ...
  return (
    ...
    >
      {offer && (
        <div
          className={`absolute top-0 left-0 bg-brand text-white ${compact ? "px-2 py-1 text-[8px]" : "px-2.5 py-1.5 text-[9px]"} font-semibold z-10 flex items-center gap-1 shadow-md uppercase tracking-tighter rounded-br-xl`}
        >
      ...
      {sameAsMenuPrice && (
        <div
          className={`absolute top-2 right-2 bg-brand-dark text-white ${compact ? "px-1.5 py-0.5 text-[7px]" : "px-2 py-0.5 text-[8px]"} font-semibold z-20 flex items-center gap-1 shadow-sm uppercase tracking-tighter border border-white/20 rounded-full`}
        >
      ...
      >
        ...
        <div className="absolute bottom-2 right-2 bg-white/90 backdrop-blur-md px-2 py-1 rounded-lg border border-slate-100 shadow-sm flex items-center gap-1">
          ...
          />
          <span
            className={`${compact ? "text-[8px]" : "text-[10px]"} font-semibold text-slate-800 group-hover:text-brand transition-colors italic`}
          >
            {time || "10 MINS"}
          </span>
        </div>
      ...
      <div className={`${compact ? "space-y-0.5" : "space-y-1"}`}>
        ...
        >
          <button
            className={`bg-white border-2 border-brand text-brand ${compact ? "px-3 py-1 text-[8px]" : "px-4 py-1.5 text-[10px]"} rounded-xl font-semibold uppercase tracking-widest hover:bg-brand hover:text-white transition-all shadow-sm`}
          >
            {price ? "Add" : "View"}
          </button>
        </div>
