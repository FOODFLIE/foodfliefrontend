...
const VariantModal = ({ isOpen, onClose, item, onConfirm, isAdding }) => {
  ...
  useEffect(() => {
    if (isOpen && item && item.variants && item.variants.length > 0) {
      ...
      const firstAvailable = item.variants.find(v => v.is_available) || item.variants[0];
      ...
      <div className="relative bg-white w-full sm:max-w-md rounded-t-[32px] sm:rounded-[32px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-300 flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto pb-32">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                 {item.is_veg !== undefined && (
                   <div className={`w-3.5 h-3.5 border ${item.is_veg ? 'border-green-500' : 'border-red-500'} flex items-center justify-center rounded-sm`}>
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${
                    ...
                    {selectedVariant?.id === v.id && <div className="w-1.5 h-1.5 bg-white rounded-full shadow-sm" />}
                  </div>
                  <span className={`font-black text-sm transition-colors ${
                    selectedVariant?.id === v.id ? 'text-brand' : 'text-slate-700'
                  }`}>
                    {v.name}
                  </span>
                </div>
                ...
                <span className={`font-display text-sm font-black ${
                  selectedVariant?.id === v.id ? 'text-brand' : 'text-slate-500'
                }`}>
                  ₹{v.price}
                </span>
              </label>
          ...
          <div className="px-6 pb-4">
             <p className="text-[10px] text-slate-400 font-bold">Recommended pairings for your {item.name}</p>
          </div>
